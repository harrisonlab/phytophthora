...
echo "---------------------------------------------------------------"
echo "Filter the SNPs in diploid and haploid genomes. Removes all SNPs. Can allow a certain number of individuals with N sites."
...
$vcftools/vcftools --vcf temp.vcf --max-missing $na --keep-only-indels --recode --out ${filename%.vcf}_filtered
mv ${filename%.vcf}_filtered.recode.vcf ${filename%.vcf}_filtered_only_indels.vcf
rm *temp*.vcf
